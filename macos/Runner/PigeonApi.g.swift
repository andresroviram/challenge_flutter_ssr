...
protocol NotificationCallbackProtocol {
  func onNotificationReceived(payload payloadArg: NotificationPayload, completion: @escaping (Result<Void, PigeonError>) -> Void)
}
class NotificationCallback: NotificationCallbackProtocol {
  ...
  func onNotificationReceived(payload payloadArg: NotificationPayload, completion: @escaping (Result<Void, PigeonError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.challenge_flutter_ssr.NotificationCallback.onNotificationReceived\(messageChannelSuffix)"
